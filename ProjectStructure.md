...
```
  - **config/**
    - roomservice-oriole.xml (180 bytes)
      - Content preview:
```
...
```
    - roomservice-star2lte.xml (467 bytes)
      - Content preview:
```
<?xml version="1.0" encoding="UTF-8"?>
<manifest>
...
```
    - **standardApps/**
      - **handheldProduct/**
        - handheld_product.mk (1329 bytes)
          - Content preview:
```
#
...
```
  - docker-compose-full-build.yml (343 bytes)
    - Content preview:
```
version: "3"
services:
...
```
- ProjectStructure.md (27013 bytes)
  - Content preview:
```
# Project Structure for /Users/oliverstaub/gitroot/LudditeOSBashScripts
...
- **build/**
  - **config/**
    - roomservice-oriole.xml (291 bytes)
      - Content preview:
` ` `
<?xml version="1.0" encoding="UTF-8"?>
<manifest>
    <remote name="TheMuppets" fetch="https://github.com/" revision="refs/heads/lineage-22.1" />
...
` ` `
  - docker-compose-full-build.yml (345 bytes)
    - Content preview:
` ` `
version: "3"
services:
